...
float pid_calculate(PID_t *pid, float sp, float val, float val_dot, float dt)
{
  ...
  if (pid->ki > SIGMA)
  {
    // Calculate the error integral and check for saturation计算误差积分并检查饱和
    i = pid->integral + (error * dt);

    /* check for saturation 检查是否饱和*/
    if (isfinite(i))
    {
      if ((pid->output_limit < SIGMA || (fabsf(output + (i * pid->ki)) <= pid->output_limit)) &&
          fabsf(i) <= pid->integral_limit)
      {
        /* not saturated, use new integral value未饱和，使用新的积分值 */
        pid->integral = i;
      }
    }

    /* add I component to output 将I组件添加到输出*/
