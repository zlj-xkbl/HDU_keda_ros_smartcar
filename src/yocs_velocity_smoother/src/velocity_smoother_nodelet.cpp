...
void VelocitySmoother::reconfigCB(yocs_velocity_smoother::paramsConfig &config, uint32_t level)
{
  ...
  teb_flage = config.teb_flage;
  decel_lim_v  = decel_factor*accel_lim_v;
  decel_lim_w  = decel_factor*accel_lim_w;
  locker.unlock();
}

void VelocitySmoother::velocityCB(const geometry_msgs::Twist::ConstPtr& msg)//检测速度
{
  ...
  //估计命令频率；我们不停地这样做，因为这可能是非常不同的取决于
//发布者类型，我们不想强加额外的约束来保持此包的灵活性
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
        ...
        ((ros::Time::now() - last_cb_time).toSec() > std::min(3.0*cb_avg_time, 0.5)))
    {
      ...
      //速度输入不再有效；通常最后一个命令是零速度命令，但请放心
//万一我们的意见出了问题，或者他忘了礼貌。。。
//问题2，额外检查cb#u avg#u时间非常大的情况，例如使用几个原子命令
//cbèu avgèu time>0检查需要处理低速率模拟时间，这可以使
...
//多条消息同时到达，因此导致中值为零
    ...
    //don't care about min / max velocities here, just for rough checking//检查反馈是否与我们期望的不符

//这里不关心最小/最大速度，只是粗略检查一下accel_lim_w
    ...
    std::cout <<"angular_max_deviation:"<<angular_max_deviation<<","<<
    "w_deviation_lower_bound:"<<w_deviation_lower_bound<<","<<
    "w_different_from_feedback"<<w_different_from_feedback<<std::endl;
    if (((robot_feedback != NONE) && (input_active == true) && (cb_avg_time > 0.0) &&
        (((ros::Time::now() - last_cb_time).toSec() > 5.0*cb_avg_time)     || // 5 missing msgs
            v_different_from_feedback || w_different_from_feedback) )|| teb_flage)
    {
      ...
      //如果发布者已经有一段时间不活动了，或者如果我们当前的命令有很大不同
//从机器人速度反馈来看，我们不能相信前者；而是依靠机器人的反馈
//如果它有很高的延迟，使用里程表可能无法很好地工作
//  decel_lim_v = decel_factor*accel_lim_v;
 // decel_lim_w = decel_factor*accel_lim_w
      ...
      if ( !quiet ) {
        ...
        //这种情况可能是不可避免的，因为抢占当前的速度控制上
  //速度多路复用器，所以安静，如果我们这样做的指示
      ...
      last_cmd_vel = current_vel;//机器人速度(pp)
    }
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      //计算矢量A（期望速度增量）和B（最大速度增量），并将其归一化，
//其中v作为坐标x，w作为坐标y；从A到B的角的符号决定
//哪个速度（v或w）必须过约束以保持指令所提供的方向
      ...
      {
        // overconstrain angular velocity 过度约束
      ...
      if (std::abs(v_inc) > max_v_inc)
      {
        ...
        if(teb_flage) cmd_vel->linear.x  = target_vel.linear.x;
      }
      ...
      if (std::abs(w_inc) > max_w_inc)
      {
        ...
        if(teb_flage) cmd_vel->angular.z = target_vel.angular.z;
      }
    ...
    {
      // We already reached target velocity; just keep resending last command while input is active我们已经达到了目标速度；只需在输入处于活动状态时继续重新发送最后一个命令
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  nh.param("teb_flage",teb_flage,false);
