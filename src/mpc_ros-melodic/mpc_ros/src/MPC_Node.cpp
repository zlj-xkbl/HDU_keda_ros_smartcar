...
class MPCNode
{
    ...
    private:
        ...
        string _map_frame, _odom_frame, _car_frame,_car_pose;
...
MPCNode::MPCNode()
{
    ...
    pn.param("thread_numbers", _thread_numbers, 2); // number of threads for this ROS node 此节点的线程数
    ...
    pn.param<std::string>("car_pose", _car_pose, "/amcl_pose" );
    ...
    _sub_amcl   = _nh.subscribe(_car_pose, 5, &MPCNode::amclCB, this);
    //_sub_amcl   = _nh.subscribe("/amcl_pose", 5, &MPCNode::amclCB, this);
    _pub_globalpath  = _nh.advertise<nav_msgs::Path>("/global_path", 1); // Global path generated from another source 从另一个源生成的全局路径
    _pub_odompath  = _nh.advertise<nav_msgs::Path>("/mpc_reference", 1); // reference path for MPC ///mpc_reference  MPC的引用路径///MPC\U引用
    _pub_mpctraj   = _nh.advertise<nav_msgs::Path>("/mpc_trajectory", 1);// MPC trajectory output 输出的轨迹
