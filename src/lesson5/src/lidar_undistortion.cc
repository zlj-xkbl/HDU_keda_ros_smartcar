...
LidarUndistortion::LidarUndistortion() : private_node_("~")
{
    ...
    if (use_imu_)
    {
        imu_subscriber_ = node_handle_.subscribe(
            "imu/data", 2000, &LidarUndistortion::ImuCallback, this, ros::TransportHints().tcpNoDelay());
    }

    if (use_odom_)
    {
        odom_subscriber_ = node_handle_.subscribe(
            "/rf2o_laser_odometry/odom", 2000, &LidarUndistortion::OdomCallback, this, ros::TransportHints().tcpNoDelay());
    }
